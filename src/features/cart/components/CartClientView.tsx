  ...
  return (
    <div className="min-h-screen bg-white pb-24 relative">
      <main className="page-max-width px-4 sm:px-6 pt-10">
        <h1 className="text-font-6 font-bold text-[var(--mama-brown)] mb-8">Keranjang</h1>

        {items.length > 0 && <FreeShippingBar missingAmount={missingForFreeShipping} progress={freeShippingProgress} />}
        ...
        {isLoading && items.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h2 className="text-font-4 font-bold text-[var(--mama-brown)] mb-2">Keranjang Mama masih kosong</h2>
            <p className="text-font-2 text-[var(--color-gray)] mb-8 max-w-md">Yuk, mulai belanja dan temukan berbagai produk ASI booster terbaik untuk perjalanan mengasihi Mama!</p>
            <a href="/products" className="bg-[var(--mama-hot-pink)] text-white px-8 py-3 rounded-full font-bold text-font-2 shadow-md hover:opacity-90 transition-all">
              Mulai Belanja
            </a>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-7 xl:col-span-8">
              <div className="flex justify-end items-center py-4 mb-2">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-font-2 font-medium text-[var(--mama-brown)]">{totalQuantity} items</span>
                  <button onClick={handleRemoveSelected} disabled={selectedIds.size === 0} className="text-[var(--mama-brown)] hover:text-[var(--mama-hot-pink)] disabled:opacity-50 transition-colors p-1">
                    <Trash2 size={22} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-col">
                {items.map((item) => (
                  <CartItemCard key={item.id} item={item} onUpdateQty={updateQuantity} onRemove={removeItem} />
                ))}
              </div>
            </div>
