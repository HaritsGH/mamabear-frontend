...
export function CheckoutView({ initialAddresses, userEmail }: CheckoutViewProps) {
  const { addresses, cart, isLoadingCart, selectedAddressId, shippingOptions, selectedShipping, isCalculatingShipping, isSubmitting, errorMessage, totals, notes, actions } = useCheckout(initialAddresses, userEmail);
  ...
  if (!isLoadingCart && (!cart || cart?.items?.length === 0)) {
    return (
      <div className="text-center py-20 text-gray-500 max-w-md mx-auto">
        <p className="mb-4">Keranjang Anda kosong. Silakan belanja terlebih dahulu.</p>
        <Link href="/products" className="inline-block bg-[var(--mama-hot-pink)] hover:bg-[#c24467] text-white font-bold py-3 px-6 rounded-full transition-colors text-font-2">
          Mulai Belanja
        </Link>
      </div>
    );
  ...
  return (
    <div className="w-full">
      ...
      {errorMessage && <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-600 rounded-lg text-font-2">{errorMessage}</div>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-8">
          ...
          <section className="space-y-4">
            <h2 className="text-font-3 font-bold text-[var(--mama-brown)]">Dikirim ke</h2>
            ...
            <div className="space-y-4">
              {addresses.map((address) => {
                ...
                return (
                  <div
                    key={address.id}
                    onClick={() => actions.handleSelectAddress(String(address.id))}
                    className={`p-5 rounded-xl border-2 cursor-pointer transition-colors relative bg-white ${isSelected ? "border-[var(--mama-hot-pink)]" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Link href={`/account/addresses`} className="text-gray-400 hover:text-[var(--mama-brown)] text-sm font-medium">
                        Ubah
                      </Link>
                    </div>
                    <div className="pl-7 space-y-1">
                      ...
                      <div className="text-gray-600 text-font-1 leading-relaxed mt-2">
                        <p>
                          {address.road} {address.detail && `(${address.detail})`}
                        </p>
                        <p>
                          Kec. {address.districtName}, Kel. {address.subdistrictName}
                        </p>
                        <p>
            ...
            <Link href="/account/addresses/new" className="block w-full text-center bg-[var(--mama-pink)] hover:bg-[#f5b8c9] text-[var(--mama-brown)] font-bold py-3 px-4 rounded-full transition-colors text-font-2">
              + Alamat baru
            </Link>
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="relative">
              <select
                ...
                disabled={isLoadingCart || isCalculatingShipping || !selectedAddressId || shippingOptions.length === 0}
                value={shippingOptions.findIndex((o) => o.service === selectedShipping?.service && o.code === selectedShipping?.code)}
                onChange={(e) => actions.handleSelectShipping(Number(e.target.value))}
              >
                {isCalculatingShipping ? (
                  ...
                  shippingOptions.map((opt, idx) => (
                    <option key={`${opt.code}-${opt.service}`} value={idx}>
                      {opt.code.toUpperCase()} - {opt.service} ({formatRupiah(opt.cost)})
                    </option>
                  ))
                )}
              </select>
          ...
          {/* Notes Section */}
          <section className="space-y-4">
            <h2 className="text-font-3 font-bold text-black">Catatan Pesanan</h2>
            <textarea
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <div className="p-5 space-y-5 border-b border-gray-200">
              {isLoadingCart
                ...
                : cart?.items?.map((item) => {
                    const itemImage = item.variant?.images?.[0]?.imageUrl || item.product?.images?.[0]?.imageUrl || "/images/placeholder.png";
                    ...
                    return (
                      <div key={item.id} className="flex gap-4 items-start">
                        <div className="w-16 h-16 bg-pink-50 rounded-md shrink-0 flex items-center justify-center border border-pink-100 overflow-hidden relative">
                          <Image src={itemImage} alt={item.product?.name || "Product image"} fill className="object-cover" />
                        </div>

                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between gap-2">
                            <h4 className="text-font-1 font-bold text-black leading-snug line-clamp-2">{item.product?.name}</h4>
                            <span className="text-font-1 font-bold text-black whitespace-nowrap">Qty {item.quantity}</span>
                          </div>
                          <p className="text-font-1 text-gray-500 mt-1 font-medium">{item.variant?.name}</p>
                          <p className="text-font-2 font-bold text-red-600 mt-2 text-right">{formatRupiah(parseFloat(item.price) * item.quantity)}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="p-5 space-y-3 bg-white">
              <div className="flex justify-between text-font-2">
                ...
                {isLoadingCart ? <div className="h-5 bg-gray-200 rounded w-20 animate-pulse"></div> : <span className="font-bold text-black">{formatRupiah(totals.subtotal)}</span>}
              </div>
              <div className="flex justify-between text-font-2">
            ...
            <div className="bg-[var(--mama-hot-pink)] p-5 flex justify-between items-center text-white">
              ...
              {isLoadingCart || isCalculatingShipping ? <div className="h-6 bg-pink-400 rounded w-24 animate-pulse"></div> : <span className="text-font-4 font-bold">{formatRupiah(totals.grandTotal)}</span>}
            </div>
          </div>
