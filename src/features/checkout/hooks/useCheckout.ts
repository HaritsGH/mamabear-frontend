...
import { validatePromo, consumePromo } from "@/features/admin/promos/services/PromoService";
...
export function useCheckout(initialAddresses: Address[], userEmail: string) {
  ...
  // Form & Data States
  const [selectedAddressId, setSelectedAddressId] = useState<string | null>(initialAddresses.length > 0 ? String(initialAddresses[0].id) : null);
  ...
  const [promoIsPercentage, setPromoIsPercentage] = useState(false);

  const appliedDiscount = (cost: number) => {
    if (!promoCode || cost <= 0) return 0;
    const raw = promoIsPercentage ? Math.round((cost * promoAmount) / 100) : promoAmount;
    return Math.min(cost, raw);
  };

  // Derived Values
  const selectedAddress = initialAddresses.find((a) => String(a.id) === selectedAddressId);
  const subtotal = cart?.items?.reduce((sum, item) => sum + parseInt(item.price as string) * item.quantity, 0) || 0;
  const tax = cart?.taxIdr || 0;
  const rawShippingCost = selectedShipping?.cost || 0;
  const shippingCost = rawShippingCost - appliedDiscount(rawShippingCost);
  const promoDiscount = promoCode ? appliedDiscount(rawShippingCost) : 0;
  const grandTotal = subtotal + shippingCost + tax;

  useEffect(() => {
    const promoFromUrl = searchParams.get("promo");
    if (!promoFromUrl) return;
    validatePromo(promoFromUrl)
      .then((result) => {
        if (result.valid) {
          setPromoCode(promoFromUrl.toUpperCase());
          setPromoAmount(result.discountAmount ?? result.amount ?? 0);
          setPromoIsPercentage(result.isPercentage ?? false);
        } else {
          toast.error("Promo tidak berlaku", {
            description: result.message || "Kode promo tidak valid atau sudah digunakan.",
          });
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
        ...
        if (options.length > 0) {
          ...
          try {
            const updatedCart = await updateCartItemCourier(cart.id, {
              shippingCostIdr: defaultOption.cost - appliedDiscount(defaultOption.cost),
    ...
    getShippingOptions();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedAddressId, cart?.id, cart?.items?.length, promoCode]);
  ...
  const handleSelectShipping = async (optionIndex: number) => {
    ...
    if (cart && selectedOption) {
      try {
        const updatedCart = await updateCartItemCourier(cart.id, {
          shippingCostIdr: selectedOption.cost - appliedDiscount(selectedOption.cost),
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (promoCode) {
        const promoRes = await consumePromo(promoCode);
        if (!promoRes.valid) {
          toast.warning("Perhatian", { description: promoRes.message || "Promo gagal diklaim." });
        }
      }
      ...
      router.push(`/checkout/payment/${order.id}?payUrl=${encodeURIComponent(transaction.paymentRedirectUrl)}`);
    } catch (error) {
      console.error("Checkout process failed:", error);
      setErrorMessage(error instanceof Error ? error.message : "Terjadi kesalahan saat memproses pesanan Anda.");
