...
export const PromoEditForm = ({ initialData }: PromoEditFormProps) => {
  ...
  const onSubmit = async (data: PromoFormValues) => {
    try {
      setError(null);
      await adminPromoService.updatePromo(initialData.code, {
        ...data,
        code: data.code.trim().toUpperCase(),
        amount: Number(data.amount),
      });
      toast.success(`Promo ${data.code} berhasil diperbarui`);
      router.push("/admin/promos");
      router.refresh();
    } catch (err) {
      console.error("Update promo error:", err);
      setError(err instanceof Error ? err.message : "Terjadi kesalahan saat memperbarui promo.");
    }
  };
