...
import { Home, LayoutGrid, ReceiptText, User, MessageCircleMore } from "lucide-react";
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-stone-100 shadow-[0_-1px_4px_rgba(214,85,126,0.5)] pb-safe">
      <div className="flex items-center justify-around h-16">
        <Link href="/" className="flex flex-col items-center justify-center w-full h-full text-[var(--mama-brown)] hover:text-primary transition-colors">
          <Home className="w-6 h-6" strokeWidth={2.5} />
          <span className="text-[10px] mt-1 font-semibold">Beranda</span>
        </Link>

        <Link href="/products" className="flex flex-col items-center justify-center w-full h-full text-[var(--mama-brown)] hover:text-primary transition-colors">
        ...
        <Link href="/account/orders" className="flex flex-col items-center justify-center w-full h-full text-[var(--mama-brown)] hover:text-primary transition-colors">
          <ReceiptText className="w-6 h-6" strokeWidth={2.5} />
          <span className="text-[10px] mt-1 font-semibold">Transaksi</span>
        </Link>

        <Link href={isLoggedIn ? "/account" : "/login"} className="flex flex-col items-center justify-center w-full h-full text-[var(--mama-brown)] hover:text-primary transition-colors">
